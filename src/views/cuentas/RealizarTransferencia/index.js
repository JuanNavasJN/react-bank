...
class RealizarTransferencia extends Component {
  ...
  componentDidUpdate = (prevProps, prevState) => {
    if (prevState.amount !== this.state.amount) {
      if (
        parseInt(this.state.amount.replace(/[^0-9]/gi, "")) >
        parseInt(this.state.available.replace(/[^0-9]/gi, ""))
      ) {
  ...
  handleSubmit = (e) => {
    ...
    let data = {
      ...
      amount: Number(
        this.state.amount.split(".").join("").split(",").join(".")
      ),
    };
    ...
    axios
      .post(`${ip}/transfer`, data, {
        headers: { Authorization: localStorage.getItem("token") },
      })
      .then((res) => {
        let aux =
          parseFloat(
            this.state.available.split(".").join("").split(",").join(".")
          ).toFixed(2) -
          parseFloat(
            this.state.amount.split(".").join("").split(",").join(".")
          ).toFixed(2);
        ...
        this.setState({
          ...
          amount: "",
          to: "",
          success: "La transferencia se realizó exitosamente.",
        });
      })
      .catch((error) => {
        if (error.response) {
              ...
              >
                <CRow>
                  <CCol xs="12">
                    <div style={{ marginBottom: "8px" }}>
                      ...
                      <CInputGroupText>{this.state.nro}</CInputGroupText>
                      <span style={{ color: "#4d54d4" }}>
                        Saldo disponible: {this.state.available}
                      </span>
                    </div>
                  </CCol>
                </CRow>
                ...
                <CRow>
                  <CCol xs="12" className="text-center mt-2">
                    ...
                    <CButton
                      disabled={this.state.maximum_reached}
                      type="submit"
                      color="primary"
                    >
                      {this.state.loading ? (
